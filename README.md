...
**Security toolkit for AI agent platforms — starting with OpenClaw and NanoClaw**

SafeClaw is a local web dashboard that enforces secure defaults, scans skills before installation, and gives users clear visibility into what their AI agent can access. Think of it as a security control panel for your AI agent stack.

## Why not just use NanoClaw?

NanoClaw is great for new setups — it's a lightweight, container-first alternative to OpenClaw with strong isolation out of the box. But SafeClaw helps the 250,000+ existing OpenClaw users harden what they already have. SafeClaw's skill scanner also works across both platforms since OpenClaw and NanoClaw share the same SKILL.md format, so you get threat detection regardless of which platform you're running.
...
Visual grid showing current permission levels with color-coded risk badges. Includes a circular security score gauge (A-F grading), Quick Fix buttons for risky settings, OpenClaw-specific settings (sandbox mode, gateway bind, bundled skills), a findings list grouped by severity, Migration Advisor for critically low scores, and backup/restore UI.

### Audit Log

Tracks every SafeClaw action: config reads, config writes, backup restores, and skill scans. Filterable by severity (Info/Warning/Danger), searchable by keyword, with expandable event details and a clear log button. Events auto-refresh every 10 seconds.

### Config Engine

Reads and writes OpenClaw's real `openclaw.json` schema (validated against actual OpenClaw v2026.3.13) plus a `safeclaw.json` sidecar for SafeClaw-specific settings. Automatic backup before every change. Bidirectional mapping between SafeClaw toggles and OpenClaw config values. Auto-migration from old formats.
...
- **SafeClaw Dashboard** — serves the UI and API, includes git for GitHub URL scanning

## Why This Matters

SafeClaw exists because 40,000+ OpenClaw instances were exposed due to insecure defaults. When OpenClaw agents connected to the Moltbook social network, researchers found prompt injection vectors, mass impersonation, and agents acting beyond their owners' intent. SafeClaw's permission controls and skill scanning address exactly these failure modes.
